...
Two physics systems coexist:

- `src/_engine/core/PhysicsRapier.ts` — the original, mesh-coupled system (17+ importers across `app`/`toolkit`). No concept of `workerTarget`; always runs on the main thread. Legacy — don't build new features on it; existing importers are a separate future migration.
- The engine-agnostic Physics API — `PhysicsAPI.ts` (facade) + `Physics/EngineRapier.ts` (Rapier backend) + `Physics/PhysicsAPITypes.ts` (shared types/protocol) + `PhysicsManager.ts` (ECS integration) + `workers/physicsWorker.ts`/`workers/physics/physicsSwitch{World,Rigid,Coll}.ts` (worker-thread RPC switchboard). This is the one to build on. Rapier is currently the only backend (`Physics/ENGINES.ts`); a settings-driven engine choice (Jolt, Ammo, etc.) is a future extension point, not implemented.
