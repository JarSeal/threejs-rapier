...
import { getSkyBoxTextureIds } from '../SkyBox';
 ...
 * registered material or sky box uses. SceneLoader runs this for the previous scene once the
 * next one has loaded, so whatever the next scene shares has been re-tagged to it (or has taken
 * refs) and stays.
 *
...
export const releaseSceneOwnedAssets = (sceneId: string) => {
  ...
  // @TODO: release sky box textures too, once replaced sky boxes dispose their PMREM nodes (a
  // reloaded source texture otherwise leaves the old one's PMREM generator on the GPU)
  const skyBoxTextureIds = getSkyBoxTextureIds();
  for (const [id, entry] of Object.entries(getTextureRegistry())) {
    if (entry.persistent || !isOwned(entry.resource)) continue;
    if (skyBoxTextureIds.has(id) || isTextureUsedByAnyMaterial(entry.resource)) continue;
    deleteTexture(id);
